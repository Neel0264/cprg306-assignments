...
// Function to fetch meal ideas from TheMealDB API based on an ingredient
async function fetchMealIdeas(ingredient) {
  try {
    ...
    return data.meals; // The API returns meals in a 'meals' array
  } catch (error) {
...
// Function to fetch details for a specific meal by ID
async function fetchMealDetails(mealId) {
  try {
    ...
    const data = await response.json();
    return data.meals ? data.meals[0] : null; // The API returns meals in a 'meals' array, take the first one
  } catch (error) {
